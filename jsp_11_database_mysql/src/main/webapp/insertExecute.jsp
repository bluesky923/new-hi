...
<body>
	<h2>JDBC CONNEXTION &amp; TEST</h2>
	<!-- JDBC class import-->
	<%@ page import="java.sql.*" %>
	<%
		// insertForm에서 전달된 파라미터 값들
		String id  = request.getParameter("id");
		String pass = request.getParameter("pass");
		String name = request.getParameter("name");
		String addr = request.getParameter("addr");
		
		// 인증된 계정 정보로 지정된 위치에 있는 데이터베이스의
		// 연결정보를 저장하는 객체
		...
		// CallcbleStatement 	- Stored Procedure저장 프로시저
		...
		try{
			...
			// 등록된 커리에 대입될 데이터 삽입 완료 = 쿼리 생성
			
			// table의 행 정보를 변경 실행 한 후
			// 변경(실행)된 행의 개수를 int타입으로 반환
			int result = pstmt.executeUpdate();
			
			if(result == 1){
				out.println("회원정보 등록 완료");
			}else{
				out.println("회원 정보 등록 실해");
			}
			out.println(" - " + result);
